...
// This implementation was contributed by the following people:
// Bruno Rosa <bruno.rosa@eldorado.org.br>
// Gustavo Serra Scalet <gustavo.scalet@eldorado.org.br>
// Igor Nunes <igor.nunes@eldorado.org.br>

// Support to Big Endian by:
// Martin Doerr <martin.doerr@sap.com>
...
void MacroAssembler::sha256_load_w_plus_k_vec(const Register buf_in,
  ...
  lvx    (kpws[0], k);
  addi   (tmp, k, 16);
  for (int n = 1; n < total_kpws-1; n++) {
    VectorRegister kpw = kpws[n];

    lvx  (kpw, tmp);
    addi (tmp, tmp, 16);
  }
  lvx  (kpws[total_kpws-1], tmp);
...
void MacroAssembler::sha256_update_sha_state(const VectorRegister a,
  ...
  Register of16       = R8;
  Register of32       = R9;
  Label state_load_aligned, after_state_load_aligned;
...
void MacroAssembler::sha256(bool multi_block) {
  ...
  Register k = R7;
  load_const_optimized(k, const_cast<uint32_t *>(round_consts), R0);
...
void MacroAssembler::sha512(bool multi_block) {
  ...
  Register k = R9;
  load_const_optimized(k, const_cast<uint64_t *>(round_consts), R0);
